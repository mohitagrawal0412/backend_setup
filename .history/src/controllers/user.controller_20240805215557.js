...
const logoutUser = asyncHandler(async (req, res) => {
  await User.findByIdAndUpdate(
    ...
    req.user._id,
    {
      ...
      // better idea is to sue like that , using undefined and null sometimes not goodd  for prouction
      // $unset: {
      //   refreshToken: 1,
      // },
    },
    { new: true },// to return the updated document after quering findByIdAndUpdate and findOneAndUpdate 
  );
  const options = {
    httpOnly: true,
    secure: true,
...
const getWatchHistory = asyncHandler(async (req, res) => {
  ...
  return (
    res.status(200).user[0].watchHistory, "Watch History fecthed successfully"
  );
});
